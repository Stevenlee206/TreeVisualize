...
public class SplayTree extends BinarySearchTree {
    ...
    public void delete(int value) {
        // 1. Tìm node cần xóa thông qua logic của lớp cha
        BinaryTreeNode nodeToDelete = (BinaryTreeNode) super.search(value);
        if (nodeToDelete == null) return; 

        // 2. Đưa node cần xóa lên làm gốc (Splay to Root)
        notifyEvent(SplayEvent.SPLAY_START, nodeToDelete);
        splay(nodeToDelete);

        // 3. Tách cây thành hai cây con: trái và phải
        BinaryTreeNode leftSubtree = (BinaryTreeNode) nodeToDelete.getLeftChild();
        BinaryTreeNode rightSubtree = (BinaryTreeNode) nodeToDelete.getRightChild();

        // Ngắt kết nối node bị xóa khỏi các nhánh
        if (leftSubtree != null) leftSubtree.setParent(null);
        if (rightSubtree != null) rightSubtree.setParent(null);

        // 4. Hợp nhất (Join) lại để tạo cây mới
        if (leftSubtree == null) {
            this.root = rightSubtree; // Nếu không có nhánh trái, nhánh phải lên làm gốc
        } else {
            // Gán tạm gốc là cây con trái để splay tìm node lớn nhất (Max Left)
            this.root = leftSubtree;
            BinaryTreeNode maxLeft = findMax(leftSubtree);
            splay(maxLeft); // Node lớn nhất cây trái giờ đã là root mới
            
            // Ép kiểu root sang BinaryTreeNode để gắn cây con phải vào
            ((BinaryTreeNode) this.root).setRightChild(rightSubtree);
            if (rightSubtree != null) {
                rightSubtree.setParent((BinaryTreeNode) this.root);
            }
        }

        notifyStructureChanged(); // Cập nhật giao diện visualization
    }

    private BinaryTreeNode findMax(BinaryTreeNode node) {
        while (node.getRightChild() != null) {
            node = (BinaryTreeNode) node.getRightChild();
        }
        return node;
    }
